...
class SimilarWorkOtherClientsControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = FakeEmptyCacheMapDataRetrievalAction,
                requireUserType: UserTypeRequiredAction = FakeUserTypeRequiredSuccessAction) = new SimilarWorkOtherClientsController(
  ...
  "SimilarWorkOtherClientsController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val validData = Map(SimilarWorkOtherClientsPage.toString -> Json.toJson(true))
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val answers = userAnswers.set(SimilarWorkOtherClientsPage,true)
