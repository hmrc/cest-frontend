...
class FollowOnContractControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = FakeEmptyCacheMapDataRetrievalAction) = new FollowOnContractController(
  ...
  "FollowOnContractController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(FollowOnContractPage.toString -> Json.toJson(Answers(true,0)))
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val answers = userAnswers.set(FollowOnContractPage,0,true)
