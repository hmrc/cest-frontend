...
class FirstContractControllerSpec extends ControllerSpecBase {
  ...
  val formProvider = new FirstContractFormProvider()
  val form = formProvider()(fakeDataRequest, frontendAppConfig)

  val view = injector.instanceOf[FirstContractView]

  def controller(dataRetrievalAction: DataRetrievalAction = FakeEmptyCacheMapDataRetrievalAction) = new FirstContractController(
  ...
  "firstContractController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(FirstContractPage.toString -> Json.toJson(Answers(true,0)))
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val answers = userAnswers.set(FirstContractPage,0,true)
