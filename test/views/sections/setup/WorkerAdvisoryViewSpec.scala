...
import assets.messages.WorkerAdvisoryMessages
import play.api.mvc.Call
import views.behaviours.ViewBehaviours
import views.html.sections.setup.{AgencyAdvisoryView, WorkerAdvisoryView}
...
class WorkerAdvisoryViewSpec extends ViewBehaviours {
  ...
  "workerAdvisory view" must {
    ...
    "have the correct title" in {
      document.title mustBe title(WorkerAdvisoryMessages.title)
    }

    "have the correct heading" in {
      document.select(Selectors.heading).text mustBe WorkerAdvisoryMessages.heading
    }
