...
trait ViewSpecBaseNew extends GuiceAppSpecBase {

  trait BaseCSSSelectors {
    val heading = "h1:not(span.hmrc-caption)"
    val subheading = "h1 span"
    val p = (i: Int) => s"#main-content form p:nth-of-type($i)"
  ...
  def assertPageTitleEqualsMessage(doc: Document, expectedMessageKey: String, args: Any*) = {
    ...
    headers.first.text.replaceAll("\u00a0", " ") must include(messages(expectedMessageKey, args:_*).replaceAll("&nbsp;", " "))
  }
