...
class ConstraintsSpec extends AnyWordSpec with Matchers with Constraints {
  ...
  "inRange" must {
    ...
    "return Valid for an Int less than the max and greater than min" in {
      val result = inRange(0, 10, "error.length")(implicitly)(5)
      result mustEqual Valid
    }

    "return Valid for an Int = min" in {
      val result = inRange(0, 10, "error.length")(implicitly)(0)
      result mustEqual Valid
    }

    "return Valid for an Int = max" in {
      val result = inRange(0, 10, "error.length")(implicitly)(10)
      result mustEqual Valid
    }

    "return Invalid for an Int less than the min" in {
      val result = inRange(0, 10, "error.length")(implicitly)(-1)
      result mustEqual Invalid("error.length", 0, 10)
    }

    "return Invalid for an Int greater than the max" in {
      val result = inRange(0, 10, "error.length")(implicitly)(11)
      result mustEqual Invalid("error.length", 0, 10)
    }
  }
}
