...
class BusinessOnOwnAccountNavigator @Inject()(implicit appConfig: FrontendAppConfig) extends Navigator with FeatureSwitching {
  ...
  override def nextPage(page: Page, mode: Mode, cookieIndicator: Option[String] = None, lang: Option[String] = None): UserAnswers => Call = routeMap(mode).getOrElse(page, _ => IndexController.onPageLoad())
}
